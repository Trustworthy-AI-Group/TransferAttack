...
class ATTA(MIFGSM):
    ...
    def __init__(self, model_name, epsilon=16/255, alpha=1.6/255, epoch=10, decay=1., gamma=1.0, targeted=False, random_start=False, 
                norm='linfty', loss='crossentropy', device=None, attack='ATTA', **kwargs):
    ...
    def load_atta_model(self, **kwargs):
        ...
        try:
            atta_model.load_state_dict(torch.load(join(dirname(__file__), '../../path/to/checkpoints/atta_model_weight.pth')))
        except:
            print("Please download the checkpoint of the 'ATTA_Model' from 'https://drive.google.com/file/d/1gCPyGTdSad0x-VNWxx2-3MRZGkFneUTi/view?usp=sharing', and put it into the path 'path/to/chevkpoints'.")
