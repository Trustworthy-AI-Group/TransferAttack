...
class FBP(Attack):
    """
    Frequency-based Perturbation (FBP) - Generation Version
    
    This is a generation-based implementation of the FBP attack that uses 
    frequency domain perturbations with pre-trained generators.
    
    Based on the original FBP attack from gradient/fbp.py, but adapted for 
    ...
    Parameters:
        ...
        cache_dir: Directory for mask caching (default 'cache/fbp_masks')
    ...
    Example script:
        python main.py --input_dir ./path/to/data --output_dir adv_data/fbp/resnet50 --attack fbp --model=resnet50
        python main.py --input_dir ./path/to/data --output_dir adv_data/fbp/resnet50 --eval
    """
    ...
    def _ensure_freq_mask(self):
        ...
        print(f"=> Building FBP frequency mask via SHAP (top {self.top_n}) from {self.bg_dir}, num={self.bg_num}")
        try:
            mask = self._build_frequency_mask_via_shap(self.bg_dir, self.bg_num, self.block)
        except Exception as e:
            print(f"[FBP] SHAP building failed ({e}), falling back to gradient-based importance.")
